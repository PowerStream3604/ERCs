...
description: Sandboxed smart wallet for secure agentic automation with time-gated, granular check control
...
Contracts MAY implement [ERC-165](./eip-165).
...
1. The interface of `SandboxedSmartWallet` is kept with only the essentials for agentic operations. The additional capability, e.g., [ERC-4337](./eip-4337) based gas abstraction, etc is left untouched for the wallet developers to decide on their preference of the stack.
...
4. Standard interface referencing [ERC-173](./eip-173)’s `owner()` is imposed for better compatibility with ownership based tooling/sdks.
...
1. `SandboxedSmartWallet` should impose signature replay attack protection for the invoke of `invokeAgentExec()` signature to not make agentKey signature not replayable.
...
7. `SandboxedSmartWallet` can optionally impose [ERC-173](./eip-173) or [ERC-8023](./eip-8023) based ownership mechanism for secure ownership management.
